...
# ── Security model for eval() ──────────────────────────────────────────────
#
# eval() is used to evaluate push_expr and apply_expr strings that are stored
# in HA config entries at setup time.  These expressions are authored by the
# local HA user via the config flow — they are NOT received from the network
# or from any external source at runtime.
#
# Attack surface:
#   - An attacker who can write to HA's config storage could inject a
#     malicious expression.  That same attacker already has full access to the
#     HA instance, so eval() does not increase the effective attack surface.
#   - The CDN that serves MDI icons is a separate, distinct code path and does
#     not reach eval().
#
# Sandbox design:
#   __builtins__ is set to {} to block access to built-in functions/types.
#   However, this restriction can be bypassed in Python via object introspection
#   chains such as ().__class__.__bases__[0].__subclasses__().  The sandbox
#   therefore provides defense-in-depth rather than a security boundary.
#
# Mitigation:
#   The context only exposes numeric helpers and the entity state object.
#   No module objects, no class definitions, and no file/network access are
#   reachable through the exposed names.  Entity state attributes are dicts of
#   primitive values (int, float, str), not object graphs.
#
#   For the specific set of push_expr / apply_expr patterns generated by the
#   config flow (scale conversions, bool_to_1_0, colour channel extraction),
#   the sandbox is sufficient.  Custom user expressions are an accepted risk
#   consistent with the power-user audience of this integration.
#
# S4 note: binding_transforms.py uses a separate, intentionally smaller context
#   because it only evaluates the fixed expressions in the hardcoded TRANSFORMS
#   dict.  That context omits _norm/_denorm/_light_apply (not needed there) and
#   adds str (needed for the bool string comparisons in TRANSFORMS).  The two
#   contexts are documented separately to make the difference visible.
# ────────────────────────────────────────────────────────────────────────────
